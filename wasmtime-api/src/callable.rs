...
impl WasmtimeFn {
    pub fn new(store: Ref<Store>, instance: InstanceHandle, export: Export) -> WasmtimeFn {
        WasmtimeFn {
            store,
            instance,
            export,
...
impl WrappedCallable for WasmtimeFn {
    fn call(&self, params: &[Val], results: &mut [Val]) -> Result<(), Ref<Trap>> {
        ...
        let exec_code_buf = context
            ...
            .map_err(|_| Ref::new(Trap::fake()))?; //was ActionError::Setup)?;
        ...
        if let Err(message) = unsafe {
            ...
            return Err(Ref::new(Trap::new(message)));
        }
