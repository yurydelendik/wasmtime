...
use super::data::{FunctionAddressMap, ModuleAddressMap};
...
#[derive(Debug)]
struct FuncLookup {
    index: BTreeMap<WasmAddress, Box<[usize]>>,
    ranges: Box<[Range]>,
}
...
pub struct AddressTransform {
    ...
    func: BTreeMap<WasmAddress, FuncTransform>,
}
...
impl<'a> TransformRangeIter<'a> {
    fn new(func: &'a FuncTransform, addr: u64) -> Self {
        let search = func.lookup.index.range((Unbounded, Included(addr)));
        if let Some((_, range_indices)) = search.last() {
            TransformRangeIter {
                addr,
...
impl AddressTransform {
    ...
    fn find_func(&self, addr: u64) -> Option<&FuncTransform> {
        ...
        if let Some((_, func)) = self.func.range((Unbounded, Included(addr))).last() {
            if addr >= func.start {
                return Some(func);
            }
        }
...
mod tests {
    ...
    use crate::data::{FunctionAddressMap, InstructionAddressMap, ModuleAddressMap};
