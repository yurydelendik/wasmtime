...
use gimli::{AttributeValue, CompilationUnitHeader, DebuggingInformationEntry, UnitOffset};

use gimli::write;

use super::address_transform::AddressTransform;
use super::attr::{clone_die_attributes, FileAttributeContext};
use super::data::{ModuleVmctxInfo, ValueLabelsRanges};
    ...
    R: Reader,
{
    ...
    match type_entry.attr_value(gimli::DW_AT_type)? {
        Some(AttributeValue::UnitRef(ref offset)) => {
            ...
            let mut entries = unit.entries_at_offset(&abbrevs, *offset)?;
            entries.next_entry()?;
